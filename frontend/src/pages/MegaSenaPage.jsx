...
import { 
  Star, Wand2, Save, RefreshCw, Flame, Snowflake, 
...
const StrategyCard = ({ strategy, icon: Icon, title, description, selected, onSelect, recommended }) => {
  const strategyColors = {
    ...
    balanced: "border-green-500/30 bg-green-500/10 hover:bg-green-500/20",
...
const GeneratedBetCard = ({ bet, onSave, saving }) => {
  ...
  return (
    <Card className="card-megasena rounded-xl" data-testid="generated-bet">
      <CardContent className="p-4">
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-4">
          {bet.numbers.map((num) => (
            <LotteryBall key={num} number={num} variant="megasena" size="lg" />
          ))}
        </div>
...
const MegaSenaPage = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [nextRes, latestRes, statsRes] = await Promise.all([
        axios.get(`${API}/lottery/megasena/next-draw`),
        axios.get(`${API}/lottery/megasena/latest`),
        axios.get(`${API}/lottery/megasena/statistics`)
      ]);
      ...
      toast.error("Erro ao carregar dados da Mega-Sena");
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateBets = async () => {
    ...
    try {
      const response = await axios.post(
        `${API}/bets/generate?lottery_type=megasena&strategy=${selectedStrategy}&count=${betCount}`
      );
  ...
  if (loading) {
    return (
      <div className="space-y-6" data-testid="megasena-page-loading">
  ...
  return (
    <div className="space-y-8" data-testid="megasena-page">
      {/* Header */}
      <div className="flex items-center gap-4">
        <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-green-600 to-emerald-700 flex items-center justify-center glow-megasena">
        ...
        <div>
          ...
          <p className="text-slate-400">A loteria mais popular do Brasil</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Next Draw */}
        <Card className="card-megasena rounded-xl" data-testid="megasena-next-draw">
          <CardHeader>
        ...
        {/* Latest Result */}
        <Card className="glass rounded-xl" data-testid="megasena-latest-result">
          <CardHeader>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3 justify-center">
              {latestResult?.dezenas?.map((num) => (
                <LotteryBall key={num} number={num} variant="megasena" size="xl" />
              ))}
            </div>
          </CardContent>
        </Card>
      ...
      {/* Info */}
      <Card className="glass rounded-xl border-green-500/20">
        <CardContent className="p-4">
          <h3 className="font-semibold text-white mb-2">Como funciona a Mega-Sena?</h3>
          <ul className="text-sm text-slate-400 space-y-1">
            <li>• Escolha 6 números de 01 a 60</li>
            <li>• Acerte 6 números = Sena (prêmio principal)</li>
            <li>• Acerte 5 números = Quina</li>
            <li>• Acerte 4 números = Quadra</li>
            <li>• Sorteios às quartas e sábados</li>
          </ul>
        </CardContent>
      </Card>
    </div>
