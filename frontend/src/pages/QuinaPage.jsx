...
const LotteryBall = ({ number, variant = "neutral", size = "md", selected = false, onClick }) => {
  const sizeClasses = {
    sm: "w-8 h-8 text-sm",
  ...
  return (
    <button 
      onClick={onClick}
      className={`lottery-ball lottery-ball-${variant} ${sizeClasses[size]} ${selected ? "ring-2 ring-white ring-offset-2 ring-offset-zinc-950" : ""} cursor-pointer`}
...
const GeneratedBetCard = ({ bet, onSave, saving }) => {
  ...
  return (
    <Card className="card-quina rounded-xl" data-testid="generated-bet">
      <CardContent className="p-4">
        ...
        <div className="flex flex-wrap gap-3 justify-center mb-4">
          {bet.numbers.map((num) => (
            <LotteryBall key={num} number={num} variant="quina" size="lg" />
          ))}
        </div>
...
const QuinaPage = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [nextRes, latestRes, statsRes] = await Promise.all([
        axios.get(`${API}/lottery/quina/next-draw`),
        axios.get(`${API}/lottery/quina/latest`),
      ...
      toast.error("Erro ao carregar dados da Quina");
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateBets = async () => {
    ...
    try {
      const response = await axios.post(
        `${API}/bets/generate?lottery_type=quina&strategy=${selectedStrategy}&count=${betCount}`
      );
  ...
  if (loading) {
    return (
      <div className="space-y-6" data-testid="quina-page-loading">
  ...
  return (
    <div className="space-y-8" data-testid="quina-page">
      {/* Header */}
      <div className="flex items-center gap-4">
        <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-700 flex items-center justify-center glow-quina">
        ...
        <div>
          ...
          <p className="text-slate-400">Gere apostas inteligentes com análise estatística</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Next Draw */}
        <Card className="card-quina rounded-xl" data-testid="quina-next-draw">
          <CardHeader>
        ...
        {/* Latest Result */}
        <Card className="glass rounded-xl" data-testid="quina-latest-result">
          <CardHeader>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3 justify-center">
              {latestResult?.dezenas?.map((num) => (
                <LotteryBall key={num} number={num} variant="quina" size="xl" />
              ))}
            </div>
          </CardContent>
        </Card>
      ...
      {/* Info */}
      <Card className="glass rounded-xl border-violet-500/20">
        <CardContent className="p-4">
          <h3 className="font-semibold text-white mb-2">Como funciona a Quina?</h3>
          <ul className="text-sm text-slate-400 space-y-1">
            <li>• Escolha 5 números de 01 a 80</li>
            <li>• Acerte 5 números = Quina (prêmio principal)</li>
            <li>• Acerte 4 números = Quadra</li>
            <li>• Acerte 3 números = Terno</li>
            <li>• Acerte 2 números = Duque</li>
          </ul>
        </CardContent>
      </Card>
    </div>
